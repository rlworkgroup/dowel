# Contributing to dowel
We welcome all contributions to dowel.

Use this guide to prepare your contribution.

## Pull requests
All contributions to the dowel codebase are submitted via a GitHub pull request.
...
After following the standard dowel setup steps, make sure to run to install the pre-commit hooks into your repository. pre-commit helps streamline the pull request process by catching basic problems locally before they are checked by the CI.
...
## Code style
The Python code in dowel conforms to the [PEP8](https://www.python.org/dev/peps/pep-0008/) standard. Please read and understand it in detail.

### dowel-specific Python style
These are dowel-specific rules which are not part of the aforementioned style guides.
...
* Add convenience imports in `__init__.py` of a package for shallow first-level repetitive imports, but not for subpackages, even if that subpackage is defined in a single `.py` file. For instance, if an import line reads `from dowel.foo.bar import Bar` then you should add `from dowel.foo.bar import Bar` to `dowel/foo/__init__.py` so that users may instead write `from dowel.foo import Bar`. However, if an import line reads `from dowel.foo.bar.stuff import Baz`, *do not* add `from dowel.foo.bar.stuff import Baz` to `dowel/foo/__init__.py`, because that obscures the `stuff` subpackage.
...
## Testing
dowel maintains a test suite to ensure that future changes do not break existing functionality. We use TravisCI to run a unit test suite on every pull request before merging.
...
### Workflow
__dowel uses a linear commit history and rebase-only merging.__
...
* *Do* fetch upstream (`rlworkgroup/dowel`) frequently and keep your `master` branch up-to-date with upstream
...
dowel follows the git commit message guidelines documented [here](https://gist.github.com/robertpainsi/b632364184e70900af4ab688decf6f53) and [here](https://chris.beams.io/posts/git-commit/).
...
```sh
git clone git@github.com:<your_github_username>/dowel.git
cd dowel
...
### Modify CHANGELOG.md
For each release in dowel, modify [CHANGELOG.md](https://github.com/rlworkgroup/dowel/blob/master/CHANGELOG.md) with the most relevant changes from the latest release. The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), which adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
